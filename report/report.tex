...
I will be working with is a labeled Chest X-Ray images \citep{Medical}
...
\citep{GoogleyR}. This allowed me to not have to store the data in my
...
algorithms'' \citep{PML}. Tensorflow allows execution on both CPU's and
...
the input and output of these operations'' \citep{PML}. A tensor is best
understood as either a scalar, vector , matrices and so on, which all
correspond to a different rank tensors (See Figure 4 \citep{PML}). These
...
neural network looks like (See Figure 4 \citep{Investopedia}). The
...
high-level features'' \citep{PML}. So rather than just looking at each
...
each other than pixels that are far away from each other'' \citep{PML}.
...
of the field that a lot of future work can be done in \citep{Deep}.
